# Requirements Archive: v1.4 Tempered
...
- [x] **TEST-03**: Unit tests cover waveshaping output ranges, the clock ratio/alignment table, consecutive-outlier clock recovery, and swing math
...
- [x] **BUG-01**: Clock tracker recovers from >3× / <⅓× tempo jumps via consecutive-outlier counting — no permanent lockout (CODE-REVIEW #1)
- [x] **BUG-02**: x1.5 and ÷1.5 ratios align on correct beat boundaries without mid-cycle truncation, after an in-Rack audition confirms the change is desirable (CODE-REVIEW #2)
...
- [x] **PKG-01**: GPL-3.0 LICENSE file present at repo root (CODE-REVIEW #5)
...
- [x] **PKG-04**: A NOTICES/credits file inventories third-party asset licenses (shipped fonts, SVG panel art)
- [x] **PKG-05**: `make dist` produces a verified `.vcvplugin` artifact (binary + plugin.json + `res/` + LICENSE)

### IP Hardening

- [x] **IP-01**: Trial/proprietary fonts (`BCBarellTEST-Regular.otf`, `FoundationLogo.ttf`) removed from the working tree and gitignored (CODE-REVIEW #7)
- [x] **IP-02**: Trial fonts purged from full git history via `git filter-repo`, force-pushed to the still-private remote, and verified clean via a fresh clone (CODE-REVIEW #7)
- [x] **IP-03**: SVG/panel art font-outline and asset provenance confirmed acceptable for public GPL release
...
| BUG-02 | Phase 23 (audition-gated) | Complete |
...
| IP-01 | Phase 25 | Complete |
| IP-02 | Phase 25 | Complete |
| IP-03 | Phase 25 | Complete |
...
*Last updated: 2026-07-10 — v1.4 milestone close: all 28 requirements delivered and reconciled to Complete (traceability drift corrected against phase summaries + on-disk evidence: LICENSE/NOTICES present, trial fonts purged & gitignored, BUG/TEST regressions pinned, repo public, VCV issue #929 filed). BUG-03/BUG-04 manual in-Rack UAT deferred (automated regression covers both) — see STATE.md Deferred Items.*
